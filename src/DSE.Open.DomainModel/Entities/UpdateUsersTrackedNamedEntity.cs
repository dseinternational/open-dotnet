...
/// <summary>
/// Entity that exposes a non-empty <see cref="Name"/> and tracks created/updated
/// timestamps, the users responsible for those changes, and a concurrency
/// <see cref="Timestamp"/>.
/// </summary>
/// <remarks>
/// User-tracked counterpart to <see cref="NamedEntity{TId}"/>. See
/// <see cref="UpdateUsersTrackedEntity{TId}"/> for the constructor contract.
/// Concrete derived types must declare a
/// <see cref="MaterializationConstructorAttribute"/>-marked constructor that chains
/// to <see cref="UpdateUsersTrackedNamedEntity{TId}(TId, string, DateTimeOffset?, string?, DateTimeOffset?, string?, Timestamp?)"/>;
...
public abstract class UpdateUsersTrackedNamedEntity<TId> : UpdateUsersTrackedEntity<TId>, INamed
    where TId : struct, IEquatable<TId>
{
    ...
    /// </summary>
    protected UpdateUsersTrackedNamedEntity(string name)
    {
    ...
    /// </summary>
    protected UpdateUsersTrackedNamedEntity(TId id, string name)
        : base(id)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(name);
        _name = name;
    ...
    /// </summary>
    protected UpdateUsersTrackedNamedEntity(
        TId id,
        string name,
