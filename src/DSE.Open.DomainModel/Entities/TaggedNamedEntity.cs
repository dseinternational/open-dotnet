...
/// <summary>
/// A <see cref="NamedEntity{TId}"/> that is also externally addressable by a <see cref="Tag"/>.
/// </summary>
public abstract class TaggedNamedEntity<TId> : NamedEntity<TId>
    where TId : struct, IEquatable<TId>
{
    ...
    /// <paramref name="name"/>.
    /// </summary>
    protected TaggedNamedEntity(Tag tag, string name)
        : base(name)
    {
    ...
    /// </summary>
    protected TaggedNamedEntity(TId id, Tag tag, string name)
        : base(id, name)
    {
        EnsureTagNotEmpty(tag);
        _tag = tag;
