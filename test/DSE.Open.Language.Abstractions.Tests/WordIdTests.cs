...
public sealed class WordIdTests
{
    ...
    // Pins the legacy hash output. Downstream systems persist WordId values
    // generated by builds predating the #384/#446 rework, all produced on
    // little-endian hardware, so the hash input must stay byte-identical:
    // [meaningId 8 bytes LE][word UTF-8][language ASCII], no separators.
    // BinaryPrimitives.WriteUInt64LittleEndian makes this stable on big-endian
    // hosts too. Any change to ordering, formatting, or separators fails here.
    [Theory]
    [InlineData(100000000001ul, "run", "en-GB", 472822766454ul)]
    [InlineData(500000000042ul, "correr", "es-ES", 297543461288ul)]
