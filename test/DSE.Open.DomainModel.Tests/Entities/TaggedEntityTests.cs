...
public class TaggedEntityTests
{
    ...
    {
        var id = Guid.NewGuid();
        var entity = new Fake(id, s_sampleTag, DateTimeOffset.UtcNow.AddDays(-1), DateTimeOffset.UtcNow, s_sampleTimestamp);
    ...
    public void Materialization_Ctor_DefaultTag_Throws()
    {
        _ = Assert.Throws<EntityDataInitializationException>(() =>
            new Fake(Guid.NewGuid(), default(Tag), DateTimeOffset.UtcNow, DateTimeOffset.UtcNow, s_sampleTimestamp));
    }
    ...
    private sealed class Fake : TaggedEntity<Guid>
    {
        ...
        public Fake(Guid id, Tag tag, DateTimeOffset? created, DateTimeOffset? updated, Timestamp? timestamp)
            : base(id, tag, created, updated, timestamp)
        {
        }
    }
}
